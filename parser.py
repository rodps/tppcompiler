...
def p_programa(p):
    ...
    p[0] = tree.Node('programa', p[1:])

def p_lista_declaracoes(p):
    '''lista_declaracoes : lista_declaracoes declaracao
                         | declaracao'''
    p[0] = tree.Node('lista_declaracoes', p[1:])

def p_declaracao(p):
    '''declaracao : declaracao_variaveis
                  | inicializacao_variaveis
                  | declaracao_funcao'''
    p[0] = tree.Node('declaracao', p[1:])

def p_declaracao_variaveis(p):
    'declaracao_variaveis : tipo DOIS_PONTOS lista_variaveis'
    p[0] = tree.Node('declaracao_variaveis', p[1:])
...
def p_inicializacao_variaveis(p):
    ...
    p[0] = tree.Node('inicializacao_variaveis', p[1:])

def p_lista_variaveis(p):
    '''lista_variaveis : lista_variaveis VIRGULA var
                       | var'''
    p[0] = tree.Node('lista_variaveis', p[1:])

def p_var(p):
    '''var : id 
           | id indice'''
    p[0] = tree.Node('var', p[1:])

def p_indice(p):
    '''indice : indice ABRE_COLCHETE expressao FECHA_COLCHETE
              | ABRE_COLCHETE expressao FECHA_COLCHETE'''
    p[0] = tree.Node('indice', p[1:])

def p_tipo(p):
    '''tipo : INTEIRO
            | FLUTUANTE'''
    p[0] = tree.Node('tipo', p[1:])
...
def p_lista_parametros(p):
    '''lista_parametros : lista_parametros VIRGULA parametro
                        ...
                        | empty'''
    if(p[1] != None):
        p[0] = tree.Node('lista_parametros', p[1:])
    else:
        p[0] = tree.Node('lista_parametros')

def p_parametro(p):
    '''parametro : tipo DOIS_PONTOS id
                 | parametro ABRE_COLCHETE FECHA_COLCHETE'''
    p[0] = tree.Node('parametro', p[1:])

def p_corpo(p):
    '''corpo : corpo acao
             | empty'''
    if(p[1] != None):
        p[0] = tree.Node('corpo', p[1:])
    else:
        p[0] = tree.Node('corpo')
...
def p_acao(p):
    '''acao : expressao
            ...
            | escreva
            | retorna'''
    p[0] = tree.Node('acao', p[1:])

def p_se(p):
    '''se : SE expressao ENTAO corpo FIM
          | SE expressao ENTAO corpo SENAO corpo FIM'''
    p[0] = tree.Node('se', p[1:])

def p_repita(p):
    'repita : REPITA corpo ATE expressao'
    p[0] = tree.Node('repita', p[1:])

def p_atribuicao(p):
    'atribuicao : var ATRIBUICAO expressao'
    p[0] = tree.Node('atribuicao', p[1:])

def p_leia(p):
    'leia : LEIA ABRE_PARENTESE var FECHA_PARENTESE'
    p[0] = tree.Node('leia', p[1:])

def p_escreva(p):
    'escreva : ESCREVA ABRE_PARENTESE expressao FECHA_PARENTESE'
    p[0] = tree.Node('escreva', p[1:])

def p_retorna(p):
    'retorna : RETORNA ABRE_PARENTESE expressao FECHA_PARENTESE'
    p[0] = tree.Node('retorna', p[1:])

def p_expressao(p):
    '''expressao : expressao_logica
                 | atribuicao'''
    p[0] = tree.Node('expressao', p[1:])

def p_expressao_logica(p):
    '''expressao_logica : expressao_simples
                        | expressao_logica operador_logico expressao_simples'''
    p[0] = tree.Node('expressao_logica', p[1:])

def p_expressao_simples(p):
    '''expressao_simples : expressao_aditiva
                         | expressao_simples operador_relacional expressao_aditiva'''
    p[0] = tree.Node('expressao_simples', p[1:])

def p_expressao_aditiva(p):
    '''expressao_aditiva : expressao_multiplicativa
                         | expressao_aditiva operador_soma expressao_multiplicativa'''
    p[0] = tree.Node('expressao_aditiva', p[1:])

def p_expressao_multiplicativa(p):
    '''expressao_multiplicativa : expressao_unaria
                                | expressao_multiplicativa operador_multiplicacao expressao_unaria'''
    p[0] = tree.Node('expressao_multiplicativa', p[1:])

def p_expressao_unaria(p):
    '''expressao_unaria : fator
                        | operador_soma fator
                        | operador_negacao fator'''
    p[0] = tree.Node('expressao_unaria', p[1:])
...
def p_operador_relacional(p):
    '''operador_relacional : MENOR
                           ...
                           | MENOR_IGUAL
                           | MAIOR_IGUAL'''
    p[0] = tree.Node('operador_relacional', p[1:])

def p_operador_soma(p):
    '''operador_soma : MAIS
                     | MENOS'''
    p[0] = tree.Node('operador_soma', p[1:])

def p_operador_logico(p):
    '''operador_logico : E_LOGICO
                       | OU_LOGICO'''
    p[0] = tree.Node('operador_logico', p[1:])

def p_operador_negacao(p):
    'operador_negacao : NEGACAO'
    p[0] = tree.Node('operador_negacao', p[1:])

def p_operador_multiplicacao(p):
    '''operador_multiplicacao : MULT
                              | DIVISAO'''
    p[0] = tree.Node('operador_multiplicacao', p[1:])
...
def p_fator(p):
    '''fator : ABRE_PARENTESE expressao FECHA_PARENTESE
             ...
             | chamada_funcao
             | numero'''
    p[0] = tree.Node('fator', p[1:])

def p_numero(p):
    '''numero : num_int
              | num_flut
              | num_notacao'''
    p[0] = tree.Node('numero', p[1:])

def p_chamada_funcao(p):
    'chamada_funcao : id ABRE_PARENTESE lista_argumentos FECHA_PARENTESE'
    p[0] = tree.Node('chamada_funcao', p[1:])
...
def p_lista_argumentos(p):
    '''lista_argumentos : lista_argumentos VIRGULA expressao
                        ...
                        | empty '''
    if(p[1] != None):
        p[0] = tree.Node('lista_argumentos', p[1:])
    else:
        p[0] = tree.Node('lista_argumentos')

def p_id(p):
    'id : ID'
    p[0] = tree.Node('ID', p[1:])

def p_num_int(p):
    'num_int : NUM_INT'
    p[0] = tree.Node('NUM_INT', p[1:])

def p_num_flut(p):
    'num_flut : NUM_FLUT'
    p[0] = tree.Node('NUM_FLUT', p[1:])

def p_num_notacao(p):
    'num_notacao : NUM_NOTACAO'
    p[0] = tree.Node('NUM_NOTACAO', p[1:])
